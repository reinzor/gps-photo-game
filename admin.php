...
$image = $mysqli->real_escape_string($_GET["image"]);

if ($action == "add_player" && $name)
{
    $generated_id = generateUniqueToken(8);
    if ($mysqli->query("INSERT INTO players (id,name) VALUES ('" . $generated_id . "', '" . $name . "')") === TRUE)
    {
...
{
    if ($mysqli->query("UPDATE players SET open=1 ") === TRUE)
    {
...
{
    if ($mysqli->query("UPDATE players SET open=0") === TRUE)
    {
...
{
    if ($mysqli->query("DELETE FROM players WHERE id='" . $id . "'") === TRUE)
    {
...
{
    if ($mysqli->query("INSERT INTO points (name,image,longitude,latitude,description) VALUES ('" . $name . "', '" . $image . "', '" . $longitude . "', '" . $latitude . "', '" . $description . "')") === TRUE)
    {
...
{
    if ($mysqli->query("DELETE FROM points WHERE id='" . $id . "'") === TRUE)
    {
        echo json_encode(array('success' => true, 'message' => "Deleted" . $id));
    }
    else
...
{
    if ($mysqli->query("DELETE FROM uploads") === TRUE)
    {
...
<table class="table">
    ...
    <?php
    // Select all qr teams
    $result = $mysqli->query("SELECT * FROM players ORDER BY name");
    if (!$result) {
