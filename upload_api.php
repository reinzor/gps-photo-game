...
{    
    if (isset($_POST['pointId']) && isset($_POST['playerId']) && isset($_POST['imageData']))
    {
        ...
        if ($mysqli->query("INSERT INTO uploads (point_id,player_id,filename) VALUES ('" . $point_id . "', '" . $player_id . "', '" . $filename . "')") === TRUE)
        {
            // Now get the number of uploads for the user:
            $num_uploads = $mysqli->num_rows($mysqli->query("SELECT id FROM uploads WHERE player_id='". $player_id ."'"));
